...
const getAllPosts = async (req, res) => {
    try {
        ...
        const postData = await PostModel.populate(
            posts,
            {
        ...
        if (posts.length === 0) {
            ...
            postData.sort((dateA, dateB) => { return dateB.createdAt - dateA.createdAt });
...
const likePost = async (req, res) => {
    try {
        ...
        if (likes) {
            ...
            if (post) {
                const liked = await PostModel.updateOne({ _id: postId }, { $push: { likes: userId } });
            } else {
...
const commentPost = async (req, res) => {
    try {
        ...
        const post = await PostModel.findById(postId);
        if (post) {
            const commentData = await PostModel.updateOne({ _id: postId }, { $push: { comments: { commentedBy: userId, comment: comment } } });
        } else {
            res.status(500).send({ message: 'Post not found' });
        }
        const posts = await PostModel.find({});
